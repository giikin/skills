---
name: apifox-mock
description: Apifox Mock 数据生成规范。当需要生成 mock 数据、Apifox Mock JSON、Apifox Mock 脚本、初始化 Mock 地址或接入 Apifox 本地代理时使用本技能。支持 Mock.js 语法 JSON 生成、高级 Mock 脚本（字段关联、条件逻辑、读取请求参数）、本地 mock 文件，以及开发环境 Mock 地址初始化。涵盖字段语义推断、类型追溯、枚举识别、代理配置约定等通用规则。
---
...
| 提到 `开启 mock 地址`、`初始化 mock 地址`、`接入 mock`、`代理到 Apifox` | Mock 地址初始化      | 修改 `.env.development` 与 `vite.config.ts`                  | [init-mock-address.md](references/init-mock-address.md)   |
