...
`/src/api` 下每个模块（如 `src/api/agent/`）包含以下内容：

| 路径                  | 说明                            | 可否手动修改                                   |
| --------------------- | ------------------------------- | ---------------------------------------------- |
| `controller/`         | 代码生成工具自动生成的接口方法  | **不可修改/新建/删除**，重新执行生成命令会覆盖 |
| `interface/`          | 代码生成工具自动生成的类型定义  | **不可修改/新建/删除**，重新执行生成命令会覆盖 |
| `request.ts`          | `createAxios` 创建的 axios 实例 | 可修改（手动维护）                             |
| `swagger.config.json` | 代码生成配置                    | 可修改（手动维护）                             |
